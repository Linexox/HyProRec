...
class QwenBackboneTest(unittest.TestCase):
    def test_thinker_forward_backward_and_roundtrip(self):
        ...
        batch = dict(
            ...
            preference_mask=torch.tensor([[True, True, False, False, False]]),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                restored.backbone.config.model_type, "qwen2_5_omni_thinker"
            )
            torch.testing.assert_close(restored(**batch).rec_scores, output.rec_scores)

    def test_omni_loader_selects_thinker(self):
        with (
            mock.patch("hyprorec.modeling_hocrs.AutoConfig.from_pretrained") as config,
            mock.patch(
                "transformers.Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained"
            ) as load,
        ):
