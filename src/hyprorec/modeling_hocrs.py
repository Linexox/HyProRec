...
class GraphTokenMoE(nn.Module):
    ...
    def __init__(
        ...
        self.residual_scales = nn.ParameterDict(
            {view: nn.Parameter(torch.tensor(residual_scale_init)) for view in views}
        )
    ...
    def forward(
        ...
        delta = (weights.to(expert_outputs.dtype).unsqueeze(-1) * expert_outputs).sum(
            dim=1
        )
...
class HoCRSRecommendationHead(nn.Module):
    """Use pooled input states as queries over one semantic item table."""
    ...
    def forward(
        ...
        preference_mask: torch.Tensor,
        item_features: torch.Tensor,
    ) -> torch.Tensor:
        if preference_mask.shape != hidden_states.shape[:2]:
            raise ValueError(
                "preference_mask must match the hidden-state batch and sequence axes."
            )
        counts = preference_mask.sum(dim=-1, keepdim=True)
        if torch.any(counts == 0):
            raise ValueError("Every sample needs at least one preference token.")

        weights = preference_mask.to(hidden_states.dtype).unsqueeze(-1)
        pooled = (hidden_states * weights).sum(dim=1) / counts.to(hidden_states.dtype)
        queries = F.normalize(self.query_projector(pooled), dim=-1)
        items = F.normalize(self.item_projector(item_features), dim=-1)
        # Candidate-wise query-key attention logits are the recommendation scores.
        return queries @ items.t() / self.temperature
...
class HoCRSModel(PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: HoCRSConfig, backbone: nn.Module | None = None) -> None:
        ...
        if config.item_table_view not in config.views:
            self.register_buffer(
                f"{config.item_table_view}_feature_table",
                torch.zeros(config.num_items, config.item_feature_dim),
                persistent=True,
            )
        ...
        self.soft_prompt_embeddings = (
            nn.Embedding(
                config.num_soft_prompt_tokens,
                lm_hidden_size,
            )
            if config.num_soft_prompt_tokens > 0
            else None
        )
    ...
    def _initialize_special_token_embeddings(self) -> None:
        ...
        if int(token_ids.max()) >= self.get_input_embeddings().num_embeddings:
            raise ValueError(
                "Resize the backbone token embeddings before creating HoCRS."
            )
        with torch.no_grad():
    ...
    def initialize_feature_tables(
        ...
        required_views = {*self.config.views, self.config.item_table_view}
        missing = required_views - set(feature_tables)
        if missing:
            raise ValueError(f"Missing feature tables: {sorted(missing)}")
        with torch.no_grad():
            for view in required_views:
                target = getattr(self, f"{view}_feature_table")
                source = feature_tables[view].to(
    ...
    def _inject_special_embeddings(
        ...
        if self.config.num_soft_prompt_tokens > 0:
            ...
            positions = torch.nonzero(
                input_ids == self.config.soft_prompt_token_id, as_tuple=False
            )
            expected = input_ids.size(0) * self.config.num_soft_prompt_tokens
            if positions.size(0) != expected:
                raise ValueError(
                    f"Expected {expected} soft prompt positions, found {positions.size(0)}."
                )
            ...
            result[positions[:, 0], positions[:, 1]] = self.soft_prompt_embeddings(
                prompt_ids
            ).to(result.dtype)
    ...
    def _inject_hypergraphs(
        ...
        for view in self.config.views:
            ...
            if hyperedge_positions.size(0) != projected.hyperedge_features.size(0):
                raise ValueError(
                    f"{view} hyperedge positions do not match encoder output."
                )
            if self.moe is not None:
            ...
            result[node_positions[:, 0], node_positions[:, 1]] = projected_nodes.to(
                result.dtype
            )
            result[hyperedge_positions[:, 0], hyperedge_positions[:, 1]] = (
                projected_edges.to(result.dtype)
            )
        ...
        if moe_weights:
            ...
            moe_view_usage = torch.stack(
                [
                    moe_view_weights.get(
                        view,
                        zero.new_zeros(MOE_NUM_EXPERTS),
                    )
                    for view in self.config.views
                ],
                dim=0,
            ...
            expert_count = MOE_NUM_EXPERTS if self.moe is not None else 0
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if preference_mask is not None:
            item_features = getattr(
                self,
                f"{self.config.item_table_view}_feature_table",
            )
            rec_scores = self.recommendation_head(
                backbone_output.hidden_states[-1],
                preference_mask,
                item_features,
            )
            if rec_labels is not None:
                rec_loss = F.cross_entropy(rec_scores, rec_labels)
        elif rec_labels is not None:
            raise ValueError(
                "preference_mask is required when rec_labels are provided."
            )
